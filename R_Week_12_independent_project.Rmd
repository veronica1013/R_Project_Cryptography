...
library(tidyverse)
knitr::opts_chunk$set(dpi=120, fig.width=6, fig.height=8)
```
...
```
The Residual Standard deviation was 1.0153. Residuals Values close to 0 are better for it tells how our model fits. The Residual standard deviation of 1.0153 indicates that our model is 1.053 difference from attaining clicks on the ad.
...
```
There's an improvement on the ETS model. it has a residual value of sigma:  0.5012 which is a bit lower from the benchmark model which had a residual Standard value of 1.0153. Most of the ACF values are within the 95% confidence interval. This means that most correlations are included in the model. Let's finally forecast using ARINA model and see what results we'll obtain. 

#### 2.4.2.3 Forecasting Using ARINA Model
```{r echo=FALSE}
fit_arima <- auto.arima(Y, d=1, D=1, stepwise = FALSE, trace=TRUE)
print(summary(fit_arima)) # The squared error was 0.4249 which residual Standard Deviation's would be 0.6518435. 
checkresiduals(fit_arima)
```
There are several ACF values outside the confidence interval. The Residual standard Deviation value for ARIMA model was a bit higher than that of Exponential Smoothing.It had a Residual standard Deviation of 0.6518435. Therefore, the best model for forecasting is Exponential Smooting as it had the lowest residual standard deviation of 0.5012 and nearly all ACF values were within the 95% confidence interval apart from one point which was slightly above the 95% confidence interval with a value of 0.01 which is insignificant.
...
print(summary(fcast))
```
Root Mean Square Error of 0.4997615 is quit low. Therefore the model prediction accuracy is good.
...
```{r}
# Plot a graph of country and clicks on ad.
#plot(factor(df$country), df$clicked_on_ad)
qplot(factor(df$city), df$clicked_on_ad)
```
```{r}
# Preview City
my_data$city
...
```
The p-value = 0.006059 is a smaller than 0.05, therefore we conclude that the distribution of the daily internet usage returns (for 2016) is significantly different from normal distribution.
...
```
From the above results, the p-value of 0.00000001754 is way below 0.05. This is a strong evidence of a strong correlation between age and the clicks on the ad.

```{r}
# Chi-Square Test of Independence between income and clicks on ad
cat("Chi-Square Test of Independence between income and clicks on ad")
chisq.test(my_data$income, my_data$clicked_on_ad)
```
A p-value of 0.4745 indicates no correlation between income and clicks on ad
...
```
There's no correlation between daily time on site and clicks on site having a p value of 0.3542.
...
```
There's also no correlation between daily internet usage and clicks on the ad having a p value of 0.4287.
...
```
The minimum daily time spent on the ad for those who clicked on it was 32.6 minutes while that of those who didn't click was 36.49 minutes with a minimum iternet usage of 149.2. Those who clicked on the ad had a minimum internet usage of 105.
...
popular_ages %>% 
  ...
  labs(
    ...
    subtitle = 'Females aged 28years old Never clicked on the ad',
    caption = 'Source: http://bit.ly/IPAdvertisingData'
  )
```
...
popular_ages %>% 
  ...
  labs(
    ...
    subtitle = 'Females aged nearing the age of 50 years old with an Income of 234,999 never Clicked on the ad',
    caption = 'Source: http://bit.ly/IPAdvertisingData'
  )
```
...
popular_ages %>% 
  ...
  labs(
    ...
    subtitle = 'Majority of the Females aged 45years and 28years old had no clicks on the ad',
    caption = 'Source: http://bit.ly/IPAdvertisingData'
  )
```
